...
Care intervals, light/humidity/temperature preferences, toxicity (ASPCA-aligned), soil pH, drainage, origin, growth rate, propagation methods, hardiness zones (USDA 1–13, 100% coverage), scientific nomenclature (POWO/Kew), and localized descriptions.
...
Each plant entry includes descriptions, care tips, seasonal watering intervals, light/humidity/temperature preferences, toxicity classification, soil pH, drainage, geographic origin, growth rate, propagation methods, and USDA hardiness zones (1–13, all 952 plants). The dataset has been audited for metadata completeness, naming overlaps, and translation sync across EN, ES, ZH-Hans, and ZH-Hant.
...
│   ├── common_plants_language_en.json       # English (typeName, description, careTips, origin)
│   ├── common_plants_language_es.json       # Spanish translations
│   ├── common_plants_language_zh-Hans.json  # Simplified Chinese translations
│   └── common_plants_language_zh-Hant.json  # Traditional Chinese translations
│
...
│   └── common_plants_zh-Hant.json      # Merged ZH-Hant
│
...
| **Files** | 1 metadata + 4 language files | 4 merged files |
| **Used by** | Scripts, contributors | App (custom URL download) |

### Source Files

**Metadata** (`common_plants_metadata.json`) — Dictionary keyed by plant ID. Contains watering intervals, light/humidity/temperature preferences, toxicity, soil pH, drainage, watering method, lifespan, category, hardiness zones, growth rate, and propagation methods.

**Language files** (`common_plants_language_*.json`) — Array of plant entries per locale. Each entry has `id`, `typeName`, `description`, `commonExamples`, `careTips`, and `origin`. The first element is a `_metadata` header with version, plant count, and category translations.
...
| `audit_translation_sync.py` | Verify EN/ES/ZH-Hans/ZH-Hant ID coverage and translation completeness |
...
| `translate_typenames.py` | Translate typeNames EN→ZH (`--lang zh-Hans`/`--lang zh-Hant`) or EN→ES (`--lang es`) |
| `convert_zh_hans_to_hant.py` | Generate ZH-Hant source by converting ZH-Hans (Simplified→Traditional Chinese) |
...
**30 categories** organized: Houseplants → Outdoor → Edibles → Farm/Sprouts → Bulbs → Specialty
...
| **Other Edibles** (5) | Fruits & Berries, Nuts & Nut Trees, Herbs, Farm & Field Crops, Sprouts & Microgreens |
| **Bulbs** (1) | Bulbs |
...
| `typeName` | string | Display name (per locale) |
| `description` | string | Plant description (ends with `.`, per locale) |
| `commonExamples` | string | Scientific names with aliases (per locale) |
| `careTips` | string | Care instructions (per locale) |
| `origin` | string | Geographic origin (per locale) |
| `category` | string | One of 30 categories |
...
| `hardinessZones` | [min, max] | USDA zones 1–13 (100% coverage) |
| `growthRate` | enum | `slow`, `moderate`, `fast` |
| `propagationMethods` | array | See below |
...
| `propagationMethods` | `stemCuttings`, `leafCuttings`, `division`, `offsets`, `layering`, `airLayering`, `seeds`, `spores`, `tuberDivision`, `bulbDivision`, `grafting`, `runners`, `plantlets` |
...
| ZH-Hans | `common_plants_zh-Hans.json` | `https://raw.githubusercontent.com/Luminoid/plantfolio-common-plants/main/dist/common_plants_zh-Hans.json` |
| ZH-Hant | `common_plants_zh-Hant.json` | `https://raw.githubusercontent.com/Luminoid/plantfolio-common-plants/main/dist/common_plants_zh-Hant.json` |
